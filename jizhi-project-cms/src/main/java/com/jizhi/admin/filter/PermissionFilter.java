...
import com.simple.common.util.LoginUserUtil;
  ...
  implements Filter
{
    ...
    throws IOException, ServletException
  {
    ...
    if (isExclude(currURI)) {
      ...
      SysUser user = (com.jizhi.admin.filter.SysUser) LoginUserUtil.getCurrentUser(request);
      if ( null == user ) {
  ...
  private void redirectLogin(HttpServletRequest request, HttpServletResponse response) {
	if (!AjaxWebUtil.isAjaxRequest(request)) {
		...
		AjaxWebUtil.sendAjaxResponse(request, response,new ResponseInfo(new ResponseStatus().notLogin(), "unlogin."));
	}
  }
}